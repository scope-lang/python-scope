...
class Parser(object):
    ...
    def p_statement_list_v(self, p):
        """statement_list_v : statement_no_semicolon
                          | statement_list statement_no_semicolon"""
        if len(p) == 2:
            p[0] = [p[1]]
        else:
            p[0] = p[1] + [p[2]]
    ...
    def p_statement_list_v_opt(self, p):
        """statement_list_v_opt : statement_list_v
                              | empty"""
        p[0] = p[1]
    ...
    def p_for_statement(self, p):
        """for_statement : FOR LPAREN statement expression SEMICOLON expression RPAREN statement"""
        # TODO : for_statement, for_var_statement, for_in_statement, for_var_in_statement
        p[0] = ast.ForStatement(first=p[3],condition=p[4], then=p[6], statement=p[8])
    ...
    def p_scope_key(self, p):
        """scope_key : SCOPE"""
        p[0] = ast.Scope()

    def p_this_key(self, p):
        """this_key : THIS"""
        p[0] =  ast.This()
    ...
    def p_call_expression(self, p):
        """call_expression : member_expression arguments
                           ...
                           | member_expression scope_arg_expression arguments
                           | call_expression scope_arg_expression arguments"""
        if len(p) == 3:
            p[0] = ast.FunctionCall(obj=p[1], argumentss=p[2],scope=None)
        else:
            #print("scope callll")
            p[0] = ast.FunctionCall(obj=p[1], argumentss=p[3],scope=p[2])


    def p_scope_arg_expression(self, p):
        """scope_arg_expression : LBRACE primary_expression RBRACE"""
        p[0] = p[2]
    ...
    def p_multiplicative_expression(self, p):
        """multiplicative_expression : unary_expression
                                     ...
                                     | multiplicative_expression TIMES multiplicative_expression
